...
**Answer** A dataframe of states that describes where their 2016 vote went and the pct of small businesses that recieved PPP loans. 
```{r}
...
**Answer** A plot of the relationship between the percentage of Trump vote within a state and the percent of businesses within that state that recieved PPP loans, along with a linear regression line running through the scatterplot. 
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** A similar plot as above, this time looking at the relationship between pct_trump and PPP amount per employee.
```{r}
...
**Answer** A table that gives us the r value for the relationship between pct_trump and businesses_receiving_ppe_pct, and pct_trump and ppe_amount_per_employee.
```{r}
reveal_data %>%
...
**Answer** Numbers that tell us the p-value, t-value, and the confidence intervals of the variables we're testing (pct_trump and businesses_receiving_ppe_pct). 
```{r}
...
**Answer** This gives us the averages for ppp_amount_per_employee and businesses_receiving_ppe_pct, based on the candidate that won in the state.
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** Numbers that give us the t-balue and p-value based on means for Trump and Clinton. 
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** Numbers that describe the t-test results for a regression based on businesses_receiving_ppe_pct and vote_2016, which suggests that there's a statistically meaningful difference between each group.
```{r}
