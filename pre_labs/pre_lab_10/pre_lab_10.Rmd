...
https://www.datawrapper.de/_/aSm2j/
...
We've created a new frame of loans by county name ! 
...
**Answer** Describe what the code below is doing and the output.
We've filtered out the loans that are less than $6 (which are likely extraneous).
```{r}
md_counties <- maryland_ppp %>%
    group_by(project_county_name) %>%
    summarise(count=n()) %>%
...
A 5 digit code for each county (state + county code). s
```{r}
...
We've joined match_county and project_county_name, giving us a cleaner frame of counties with fips codes. 
```{r}
...
The map zoomed in from the whole country to just on Maryland. The map is also... diagonal, which is strange.
...
When you hover over the map counties now, you'll see the county names and the number of loans in each county.  
...
https://www.datawrapper.de/_/PdN7V/
