...
**Answer**:
When the franchise name is NA, there are 192959 loans. When the franchise name is Subway, there are 275 loans. 
...
**Answer**:
There are 135073 loans when the forgiveness_amount is NA. 
...
**Answer**:
There are 135073 loans when the forgiveness date is NA, which is the same number for when the forgiveness amount is NA. 
...
**Answer**:
There are once again 135073 loans where forgiveness_date and forgiveness_amount is NA, which is the same amount. We can hypothesize that this number represents loans that have not been forgiven, but we would have to analyze further to confirm. 
```{r}
ppp_maryland_loans %>%
...
**Answer**
The largest loan is 10000000, which is not suspicious because that was the max loan available in the program. The smallest loan is 6, which is strange because 6 dollars would not really make a dent in any business. 
```{r}
...
**Answer**
All of the various field with amount align. 
```{r}
...
**Answer**
The amount field is a character. 
```{r}
...
**Answer**
There is a fully duplicated row, ZIP field mixes 5 digit ZIPS and 9 digit ZIPS, and the amount column is stores as a character, not a number.
...
**Answer**
It changes the columns so it no longer started with a number to x1_id. 
...
**Answer**
The error is a problem with summarise. Since the amount column is still a character, it's not possible to add up character values and summarise them like we would do with regular numbers. 
...
**Answer**
There is 1 duplicate--Stuart Jones LLC is listed twice. 
...
**Answer**
The new data frame only has 5 rows. 
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
...
**Answer**
There is one more mispelling of Arnold that remains. 
