...
il_shootings <- new_shootings_w_acs  %>%
  filter(state == "IL") %>%
  group_by(NAME) %>%
  summarize(
    count = n()
...
## surprise! cook county is overrepresented (likely because it houses chicago)
...
il_shootings <- new_shootings_w_acs  %>%
  ...
  arrange(desc(count))
### this makes sense --> 45 shootings in Chicago, but also notably 4 shootings in Rockford and 3 in Joliet and Peoria

### DC --> in what tracts are the most shootings happening?
dc_shootings <- new_shootings_w_acs  %>%
  filter(state == "DC") %>%
  group_by(NAME) %>%
  summarize(
    count = n()
  ...
  arrange(desc(count))
## not super meaningful since everything is in DC, but there are a couple of standout tracts with more than one shooting. 
...
mo_shootings <- new_shootings_w_acs  %>%
  filter(state == "MO") %>%
  group_by(NAME) %>%
  summarize(
    count = n()
...
## boone county and st. louis city are overrepresented here. 
...
nj_shootings <- new_shootings_w_acs  %>%
  filter(state == "NJ") %>%
  group_by(NAME) %>%
  summarize(
    count = n()
...
## more shootings in a tract in bergen county, which makes sense... what happens if we group by city?
...
## Newark, Camden, Atlantic City, and Paterson. Makes sense!
...
#### looking at the shootings in tracts at the county level

## all counties that have majority black and poor tracts
county_by_poverty_black_shooting <- new_shootings_w_acs_county %>%
  filter(!is.na(tract_majority_poor)) %>%
  group_by(state, county, shooting_y_n, tract_majority_poor, tract_majority_black) %>%
  summarise(
    count=n()
  ) 
...
pct_tracts_with_shootings_county<- county_by_poverty_black_shooting %>%
  ...
  ungroup() %>%
  select(state, county, majority_poor_black, count) %>%
  group_by(state, county, majority_poor_black) %>%
  summarize(
    count = sum(count)
  ) %>%
pivot_wider(names_from="majority_poor_black", values_from="count") %>%
...
#rename columns
large_pop_counties <- rename(large_pop_counties, census_tracts_geoid = GEOID, population = estimate.population)
