...
### this makes sense --> 45 shootings in Chicago, but also notably 4 shootings in Rockford and 3 in Joliet and Peoria
  ...
  ## what's the demographic breakdown of those tracts?
    ## let's string_detect for Cook County and look at the demographic breakdown of tracts that had shootings
    cook_county_shootings <- grouped_shootings_w_acs  %>%
     filter(NAME == str_detect(NAME, "~Cook County, Illinois")) %>%
      group_by(NAME) %>%
      summarize(
      count = n()
      ) %>%
